...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find user's matchup. Playoff schedule entries can carry a single side (a
    // bye), so never assume both home and away exist.
    ...
    if (!mySide || !theirSide) {
      return NextResponse.json(
        { error: 'You have a bye this matchup period — no opponent to compare against.' },
        { status: 404 },
      )
    }
